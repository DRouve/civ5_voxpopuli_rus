	...
	UPDATE Language_ru_RU SET Text = 'Основывает новые города для расширения вашей империи.[NEWLINE][NEWLINE][ICON_FOOD] Рост города останавливается, пока строится этот отряд. Уменьшает население города [ICON_CITIZEN] на 1 после завершения.[NEWLINE][NEWLINE][COLOR_NEGATIVE_TEXT]Невозможно найти города рядом с чьей-либо границей или если ваша империя очень недовольна.[ENDCOLOR]' WHERE Tag = 'TXT_KEY_UNIT_HELP_SETTLER';
	
	-- Great Merchant Text

	UPDATE Language_ru_RU SET Text = 'Если юнит находится на территории города-государства, с которым вы не воюете, этот приказ приведет к расходу юнита.[NEWLINE][NEWLINE]Вы получите большое количество золота [ICON_GOLD] и мгновенное событие «День любви короля». "во всех принадлежащих городах. Каждый город, который вы создали и которым владеете, увеличивает количество золота и продолжительность «Дня любви к королю» на 25%. Это действие поглотит единицу.' WHERE Tag = 'TXT_KEY_MISSION_CONDUCT_TRADE_MISSION_HELP';

	UPDATE Language_ru_RU SET Text = 'Великий Торговец может построить специальное городское улучшение, которое при работе дает [ICON_GOLD] золота и [ICON_FOOD] еды. Великий торговец также может отправиться в город-государство и выполнить «торговую миссию», которая принесет большую сумму золота [ICON_GOLD] и начнет «День любви к королю» во всех принадлежащих ему городах, что увеличивается на 25% за каждый созданный город. и принадлежит. Великий торговец расходуется при использовании любым из этих способов.[NEWLINE][NEWLINE]Города получают +1 [ICON_GOLD] золота и [ICON_PRODUCTION] производства, если они построены на дороге, соединяющей два принадлежащих им города, и +2 [ICON_GOLD] золота. и [ICON_PRODUCTION] Производство железной дороги. Получите дополнительное золото [ICON_GOLD] и производство [ICON_PRODUCTION] (+1 для дорог, +2 для железных дорог), если торговый путь, внутренний или международный, проходит через этот город.' WHERE Tag = 'TXT_KEY_UNIT_GREAT_MERCHANT_STRATEGY';

	UPDATE Language_ru_RU
	SET Text = '[COLOR_CYAN]Особое улучшение: город[ENDCOLOR][NEWLINE]Вы можете потратить Великого торговца, чтобы создать особое улучшение города, которое при работе дает [ICON_GOLD] золота [COLOR_YELLOW] и [ICON_FOOD] еды.[ENCDOLOR][NEWLINE] [NEWLINE][COLOR_CYAN]Особая способность: торговая миссия[ENDCOLOR][NEWLINE]Если великий торговец находится на территории города-государства, с которым вы не воюете, вы можете использовать его для выполнения торговой миссии.[NEWLINE][NEWLINE] ]Вы получите большое количество [ICON_GOLD] золота [COLOR_YELLOW] и мгновенное событие «Мы любим День короля» во всех принадлежащих вам городах. Каждый город, который вы создали и которым владеете, увеличивает количество золота и продолжительность Дня любви к королю на 25%.[ENDCOLOR] Это действие израсходует единицу.[ENDCOLOR][NEWLINE][NEWLINE]'
	...
	UPDATE Language_ru_RU SET Text = 'Этот заказ ускорит производство текущих усилий города. Каждая принадлежащая вам мануфактура, которую вы создали и которой владеете, увеличивает объем производства на 25%. Он поглощает Великого Человека.' WHERE Tag = 'TXT_KEY_MISSION_HURRY_PRODUCTION_HELP';

	UPDATE Language_ru_RU SET Text = 'Великие инженеры могут построить специальное улучшение Фабрики. Когда он работает, он обеспечивает много продукции [ICON_PRODUCTION] для города. Великий инженер также может ускорить производство юнитов, зданий или Чудес в городе, что увеличивается на 25% за каждую созданную и принадлежащую мануфактуру. Великий инженер расходуется при использовании любым из этих способов.' WHERE Tag = 'TXT_KEY_UNIT_GREAT_ENGINEER_STRATEGY';

	UPDATE Language_ru_RU
	SET Text = '[COLOR_CYAN]Особое улучшение: фабрика[ENDCOLOR][NEWLINE]Вы можете потратить великого инженера, чтобы создать фабрику. Фабрика производит огромное количество продукции (молотов) для города, если она работает.[NEWLINE][NEWLINE][COLOR_CYAN]Особая способность: ускорение производства[ENDCOLOR][NEWLINE]Вы можете потратить Великого инженера, чтобы ускорить производство на текущие усилия города. [COLOR_YELLOW]Объем производства равен 5 оборотам среднего недавнего производства лучшего города. Каждая собственная мануфактура, которую вы создали и владеете, увеличивает ее на 25%.[ENDCOLOR][NEWLINE][NEWLINE]'
	...
	UPDATE Language_ru_RU SET Text = 'Это поможет Великому Человеку исследовать технологию. Каждая собственная Академия, которую вы создали и которой владеете, увеличивает количество получаемых наук на 10%. Он поглощает Великого Человека.' WHERE Tag = 'TXT_KEY_MISSION_DISCOVER_TECH_HELP';

	UPDATE Language_ru_RU SET Text = 'Великий ученый может построить специальное улучшение Академии, которое при работе дает много [ICON_RESEARCH] Науки. Кроме того, Великий Ученый может дать вам значительный толчок к развитию вашей следующей технологии, увеличенный на 10% за каждую созданную и принадлежащую Академию. Великий Ученый расходуется при использовании любым из этих способов.' WHERE Tag = 'TXT_KEY_UNIT_GREAT_SCIENTIST_STRATEGY';

	UPDATE Language_ru_RU
	SET Text = '[COLOR_CYAN]Особое улучшение: Академия[ENDCOLOR][NEWLINE]Вы можете потратить Великого ученого, чтобы создать Академию. Во время работы Академия даст вашему городу большой научный прирост.[NEWLINE][NEWLINE][COLOR_CYAN]Особая способность: Бесплатная наука[ENDCOLOR][NEWLINE]Вы можете потратить своего Великого ученого, чтобы немедленно получить единовременную сумму науки. [COLOR_YELLOW]Каждая академия, которую вы создали и владеете, увеличивает количество получаемых наук на 10%.[ENDCOLOR][NEWLINE][NEWLINE]'
	...
	-- Great Artist
	UPDATE Language_ru_RU SET Text = 'Великий Художник может создать Великое Произведение Искусства (генерирует [ICON_CULTURE] Культуру и [ICON_TOURISM] Туризм), которое размещается в ближайшем городе, в котором есть подходящее здание с пустой ячейкой (например, дворец, музей или собор). ). Великий художник также может заработать большую сумму очков Золотого века, масштабируясь с [ICON_GOLDEN_AGE] количеством очков Золотого века, [ICON_TOURISM] доходом от туризма и количеством [COLOR_POSITIVE_TEXT]тематических[ENDCOLOR] наборов [ICON_GREAT_WORK] великих работ. Великие художники расходуются при использовании любого из этих способов.' WHERE Tag = 'TXT_KEY_UNIT_GREAT_ARTIST_STRATEGY';
	UPDATE Language_ru_RU SET Text = '+{1_Num} [ICON_GOLDEN_AGE] очков Золотого века[NEWLINE][NEWLINE]Этот заказ поглотит Великого художника и создаст очки Золотого века, что может вызвать [ICON_GOLDEN_AGE] Золотой век (дополнительное производство [ICON_PRODUCTION], [ICON_GOLD] золота, и [ICON_CULTURE] Культура). Масштаб зависит от [ICON_TOURISM] туризма и [ICON_GOLDEN_AGE] очков Золотого века за последние 15 ходов и увеличивается на 20 % за каждый принадлежащий [COLOR_POSITIVE_TEXT]тематический[ENDCOLOR] набор великих работ.' WHERE Tag = 'TXT_KEY_MISSION_START_GOLDENAGE_HELP';

	UPDATE Language_ru_RU
	SET Text = '[COLOR_CYAN]Особая способность: великолепная работа[ENDCOLOR][NEWLINE]Великий художник может создать великое произведение искусства (генерирует как [ICON_CULTURE] культуру, так и [ICON_TOURISM] туризм), которое размещается в ближайшем городе, в котором есть соответствующее здание с пустой слот (например, дворец, музей или собор). При таком использовании Великий художник тратится.[NEWLINE][NEWLINE][COLOR_CYAN]Особая способность: очки Золотого века[ENDCOLOR][NEWLINE]Великий художник можно потратить, чтобы [COLOR_YELLOW]генерировать очки Золотого века, что может вызвать [ICON_GOLDEN_AGE] Золотой век (дополнительное [ICON_PRODUCTION] производство, [ICON_GOLD] золото и [ICON_CULTURE] культура). Количество сгенерированных очков Золотого века зависит от [ICON_TOURISM] туризма и [ICON_GOLDEN_AGE] очков Золотого века за последние 15 ходов и увеличивается на 20 % за каждый принадлежащий вам тематический набор великих работ.[ENDCOLOR]'
	...
	-- Great Writer
	UPDATE Language_ru_RU SET Text = 'Великий Писатель может создать Великое Писательское произведение (генерирует как [ICON_CULTURE] Культура, так и [ICON_TOURISM] Туризм), которое размещается в ближайшем городе, в котором есть подходящее здание с пустой ячейкой (например, Амфитеатр, Национальный эпос, Героический эпос, или Королевская библиотека). Великий писатель также может написать политический трактат, который дает игроку количество культуры, зависящее от количества принадлежащих [ICON_GREAT_WORK] великих произведений. Великие Писатели расходуются при использовании любого из этих способов.' WHERE Tag = 'TXT_KEY_UNIT_GREAT_WRITER_STRATEGY';
	UPDATE Language_ru_RU SET Text = 'Этот орден поглотит Великого человека и даст большое количество [ICON_CULTURE] культуры, увеличивающееся на 3% за каждую принадлежащую Великую работу.' WHERE Tag = 'TXT_KEY_MISSION_GIVE_POLICIES_HELP';

	UPDATE Language_ru_RU
	SET Text = '[COLOR_CYAN]Особая способность: Великая работа[ENDCOLOR][NEWLINE]Великий писатель может создать великое писательское произведение (генерирует [ICON_CULTURE] культуру и [ICON_TOURISM] туризм), которое размещается в ближайшем городе, в котором есть соответствующее здание с пустой слот (например, амфитеатр, национальный эпос, героический эпос или королевская библиотека). При таком использовании великий писатель расходуется.[NEWLINE][NEWLINE][COLOR_CYAN]Особая способность: политический трактат[ENDCOLOR][NEWLINE]Великий писатель может написать политический трактат, который дает игроку большое количество [ICON_CULTURE]. Культура, [COLOR_YELLOW]увеличивается на 3 % за каждое принадлежащее вам великое произведение[ENDCOLOR]. При таком использовании Великий Писатель расходуется.'
	...
	UPDATE Language_ru_RU SET Text = 'Великий Адмирал может сразу же свободно пересекать океан, что делает его полезным (хотя и уязвимым) исследовательским кораблем. Кроме того, находясь на принадлежащей ему территории, великого адмирала можно отправить в [COLOR_POSITIVE_TEXT]путешествие открытий[ENDCOLOR], которое тратит отряд и дает вам две копии роскошного ресурса, которого нет на текущей карте.[NEWLINE][NEWLINE] Великий адмирал имеет возможность мгновенно [COLOR_POSITIVE_TEXT]ремонтировать[ENDCOLOR] каждое военно-морское и погруженное подразделение в одном и том же гексе, а также в соседних гексах. Великий адмирал расходуется при выполнении этого действия. Великий адмирал также дает +15% [ICON_STRENGTH] к боевой мощи всем морским подразделениям, принадлежащим игроку, в пределах 2 клеток. Великий адмирал НЕ расходуется, когда он предоставляет этот бонус. Когда великий адмирал тратится на [COLOR_POSITIVE_TEXT]Путешествие открытий[ENDCOLOR] или [COLOR_POSITIVE_TEXT]ремонт[ENDCOLOR], он увеличивает лимит военных юнитов на 1.' WHERE Tag = 'TXT_KEY_UNIT_GREAT_ADMIRAL_STRATEGY';

	UPDATE Language_ru_RU SET Text = 'Этот приказ поглотит великого адмирала и исцелит все ваши морские и погруженные юниты на этом и всех соседних тайлах. Это также увеличивает лимит военных юнитов на 1.' WHERE Tag = 'TXT_KEY_MISSION_REPAIR_FLEET_HELP';

	UPDATE Language_ru_RU
	SET Text = '[COLOR_CYAN]Особая способность: боевой бонус[ENDCOLOR][NEWLINE]Великий адмирал дает 15% боевой бонус всем дружественным военно-морским подразделениям в пределах 2 клеток. Этот боевой бонус распространяется на все виды морского боя: ближний и дальний бой, оборону и т. д.[NEWLINE][NEWLINE][COLOR_CYAN]Особая способность: ремонтный флот[ENDCOLOR][NEWLINE]Вы можете исцелить всех с помощью Великого адмирала. ваших военно-морских юнитов и высадившихся юнитов на этом тайле и на всех соседних тайлах. [COLOR_YELLOW]Этот приказ также увеличивает лимит военных юнитов на 1.[ENDCOLOR] Великий адмирал расходуется при использовании таким образом.[NEWLINE][NEWLINE][COLOR_CYAN]Особая способность: Путешествие открытий[ENDCOLOR][NEWLINE][ COLOR_GREEN]Вы можете отправить Великого адмирала в путешествие открытий, которое предоставит вам две копии роскошного ресурса, недоступного на текущей карте. Этот приказ также увеличивает лимит военных юнитов на 1. Великий адмирал расходуется при использовании таким образом.[ENDCOLOR]'
	...
	UPDATE Language_ru_RU
	SET Text = '[COLOR_CYAN]Особая способность: Основать религию[ENDCOLOR][NEWLINE]Если вы еще не основали религию, а религии еще можно основать, вы можете использовать Великого пророка, чтобы основать религию и выбрать для нее религиозные верования. Город, в котором основана религия, будет обращен в вашу новую религию. При таком использовании Великий пророк расходуется.[NEWLINE][NEWLINE][COLOR_CYAN]Особая способность: усиление религии[ENDCOLOR][NEWLINE]Вы можете использовать Великого пророка, чтобы усилить свою религию двумя дополнительными религиозными верованиями. При таком использовании Великий пророк расходуется.[NEWLINE][NEWLINE][COLOR_CYAN]Особое улучшение: Святое место[ENDCOLOR][NEWLINE]Великий пророк может построить особое Святое место, которое при работе дает дополнительные [ICON_PEACE] Вера. При таком использовании Великий пророк расходуется.[NEWLINE][NEWLINE][COLOR_CYAN]Особая способность: Распространение религии[ENDCOLOR][NEWLINE]Великий пророк может распространять вашу религию 4 раза, вытесняя все предыдущие религии в целевых городах. Великий Пророк расходуется при таком использовании.'
	...
	-- Inquisitor
	UPDATE Language_ru_RU SET Text = 'Можно купить за [ICON_PEACE] Веру в любом городе с преобладающей религией, которая была улучшена. Они могут удалить другие религии из ваших городов (расходуя Инквизитора) или быть размещены внутри города, чтобы уменьшить распространение миссионеров и пророков в этом городе на 50%. Снятие Ереси вызывает 1 ход сопротивления [ICON_RESISTANCE] в городе.' WHERE Tag = 'TXT_KEY_UNIT_INQUISITOR_STRATEGY';
	
	UPDATE Language_ru_RU SET Text = 'Используется для удаления других религий из городов. Инквизиторы, размещенные в городах, уменьшают распространение иностранных миссионеров и пророков на 50%. Снятие Ереси вызывает 1 ход сопротивления в городе. Можно получить только при покупке за Веру [ICON_PEACE].' WHERE Tag = 'TXT_KEY_UNIT_HELP_INQUISITOR';

	UPDATE Language_ru_RU SET Text = 'Этот приказ устранит присутствие представителей других религий в ближайшем дружественном городе. Вызывает 1 ход сопротивления в городе. Инквизитор поглощен этим действием.' WHERE Tag = 'TXT_KEY_MISSION_REMOVE_HERESY_HELP';
	-- Archaeologist Text

	UPDATE Language_ru_RU SET Text = 'Максимум [COLOR_POSITIVE_TEXT]3[ENDCOLOR] активных археологов на игрока в любой момент времени. Археологи — это особый подтип рабочих, которые используются для раскопок древностей для улучшения достопримечательностей или извлечения [ICON_GREAT_WORK] артефактов для заполнения [ICON_GREAT_WORK] слотов великих произведений искусства в музеях, дворцах, Эрмитажах и избранных чудесах. Археологи могут работать на территории, принадлежащей любому игроку. Они потребляются после завершения археологических раскопок в древности. Археологов нельзя купить за [ICON_GOLD] золота, их можно построить только в городе, где есть [COLOR_POSITIVE_TEXT]государственная школа[ENDCOLOR].' WHERE Tag = 'TXT_KEY_UNIT_HELP_ARCHAEOLOGIST';
	...
	UPDATE Language_ru_RU
	SET Text = 'Отряд должен находиться на клетке, принадлежащей другой крупной цивилизации, с которой вы не воюете.'
	...
	UPDATE Language_ru_RU
	SET Text = 'Лучник — ранний дальнобойный юнит. Это первый в линии юнитов, имеющий 2 дальности, что позволяет ему начинать драки и держаться подальше от атакующих в ближнем бою. Используйте лучников, чтобы смягчить звездные цели перед ударом в ближнем бою.'
	...
	UPDATE Language_ru_RU
	SET Text = 'Галера — это юнит варваров, который остается на тайлах побережья, ищет посаженные на корабли юниты для уничтожения. Галера может раздражать города и юниты у побережья.'
	WHERE Tag = 'TXT_KEY_UNIT_GALLEY_STRATEGY';

	UPDATE Language_ru_RU
	SET Text = 'Доминирующее военно-морское подразделение классической эпохи, владевшее морями вплоть до средневековья с помощью своей дальней атаки.'
	WHERE Tag = 'TXT_KEY_UNIT_HELP_BYZANTINE_DROMON';

	UPDATE Language_ru_RU
	SET Text = 'Атакует смертоносным греческим огнем, что делает его первым морским юнитом с дальней атакой. Он не может закончить свой ход на тайлах океана за пределами границ города.'
	...
	UPDATE Language_ru_RU
	SET Text = 'Ракетный крейсер — современный военный корабль. Он быстрый и крепкий, обладает средним ударом и неплохо перехватывает вражеские самолеты. Самое главное, ракетный крейсер может нести управляемые ракеты и ядерные ракеты, что позволяет вам доставлять это смертоносное оружие прямо к берегу противника. Ракетные крейсера в сочетании с авианосцами, подводными лодками и линкорами образуют дьявольски мощную армаду.'
